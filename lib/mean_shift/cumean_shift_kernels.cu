...
__global__ void _mean_shift_kernel_tiled_fixed(
	...
	extern __shared__ float shbuf[]; 	 // (tile_size + blockDim.x)*m elements
	float *outbuf = shbuf + blockDim.x*m; // buffer to hold the shifted points (1 point/thread)
	// shared counter for the converged threads within the same block
	int* convergence_counter = reinterpret_cast<int *>(shbuf+2*blockDim.x*m);
	...
	if( threadIdx.x == 0 ) // Only the first thread of the block resets the counter
		*convergence_counter = 0;
	__syncthreads();
	if( tid >= n ){
		thread_converged = true;
	...
	for( int s=0 ; s<max_iter && (*convergence_counter)<blockDim.x ; s++ ){
		...
		for( int t=0 ; t<(int)ceilf(n/(float)blockDim.x) ; t++ ){ // tiling loop
			...
			if( tid < n && !thread_converged ){
				// Compute the shifted point
				for( int i=0 ; i<blockDim.x && t*blockDim.x+i < n ; i++ ){
					g_val = 0;
					for( int j=0 ; j<m ; j++ )
//						g_val += powf( in[j*n+tid] - shbuf[j*blockDim.x+i] , 2.0 );
						g_val += (in[j*n+tid] - shbuf[j*blockDim.x+i])*(in[j*n+tid] - shbuf[j*blockDim.x+i]);
					g_val = expf( -0.5 * g_val/h );
					if( tid == t*blockDim.x + i )
						g_val = 0;
					g_acc += g_val;
					for( int j=0; j<m; j++ )
						outbuf[j*blockDim.x + threadIdx.x] += g_val * shbuf[j*blockDim.x+i];
		...
		if( !thread_converged ){
			...
			for( int j=0 ; j<m ; j++ ){
				outbuf[j*blockDim.x + threadIdx.x ] /= g_acc;
				// 48 regs/thread
				msv_sqnorm += powf( outbuf[j*blockDim.x+threadIdx.x] - in[j*n+tid] , 2.0 );
				// 52 regs/thread
//				msv_sqnorm += (outbuf[j*blockDim.x+threadIdx.x] - in[j*n+tid]) *
//							  (outbuf[j*blockDim.x+threadIdx.x] - in[j*n+tid]);
			}

			if( msv_sqnorm >= tol*tol ){
				for( int j=0 ; j<m ; j++ )
					out[j*n+tid] = outbuf[j*blockDim.x + threadIdx.x];
			}else{
				thread_converged = true;
				atomicAdd( convergence_counter , 1 );
			}
